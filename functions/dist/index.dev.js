...
exports.main = functions.https.onRequest(function _callee(request, response) {
  var ref, snapshot, tempWorkers, ref2, snapshot2, tempFacilities, ref3, snapshot3, tempWorkOrders, i;
  return regeneratorRuntime.async(function _callee$(_context) {
    while (1) {
      switch (_context.prev = _context.next) {
        case 0:
          ref = db.collection('workers');
          _context.next = 3;
          return regeneratorRuntime.awrap(ref.get());
        ...
        case 3:
          ...
          snapshot.forEach(function (doc) {
            ...
            console.log(tempWorkers[0].certifications); //console.log(tempWorkers[i].certifications);
          });
          console.log(tempWorkers[1].certifications[0]);
          ref2 = db.collection('facility');
          _context.next = 10;
          return regeneratorRuntime.awrap(ref2.get());

        case 10:
          snapshot2 = _context.sent;
          tempFacilities = [];
          snapshot2.forEach(function (doc) {
            //console.log(doc.id, '=>', doc.data());
            var location = [doc.data().Location.longitude, doc.data().Location.latitude];
          ...
          ref3 = db.collection('sample work order');
          _context.next = 16;
          return regeneratorRuntime.awrap(ref3.get());

        case 16:
          snapshot3 = _context.sent;
          tempWorkOrders = [];
          snapshot3.forEach(function (doc) {
            ...
            var temp3 = new WorkOrder(eid, etype, facility, location, priority, submissionTime, timeToComplete);
          ...
          for (i = 0; i < tempWorkers.length; ++i) {
            // collection should be named schedules
            // id is worker name
            // field is array of schedules
            db.collection('schedules').doc(tempWorkers[i].name).set({
      ...
      for (var j = i + 1; j < workers.length; ++j) {
        if (!workers[j].doneScheduling) {
          ...
          if (bestTaskw1.eID === bestTaskw2.eID) {
            w1SecondSchedule = oneSchedule(workers[i], removeTask(bestTaskw1, worker1Tasks));
            w2SecondSchedule = oneSchedule(workers[j], removeTask(bestTaskw1, worker1Tasks));
